...
const sendAttachement = TryCatch(async (req, res, next) => {
  ...
  const attachements = await uploadFileToCloudinary(files);
...
const renameGroup = TryCatch(async (req, res, next) => {
  ...
  if (!chat.groupChat)
    return next(new ErrorHandler("This is not a gorup chat", 400));
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  if (chat.groupChat && chat.creator.toString() !== req.user.toString())
    return next(new ErrorHandler("You are allowed to delete the group", 403));
  ...
  // Here we've to delete all messages as well as attachements or files from cloudinary
